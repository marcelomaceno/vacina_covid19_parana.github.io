...
setwd("C:/Users/Marcelosmmj/Dropbox/projetos/Dados coronavirus parana/vacina_covid19_parana.github.io")

#############################################################
#DADOS DE CASOS DE COVID                                    #
#############################################################
dados_covid <- fread("https://www.saude.pr.gov.br/sites/default/arquivos_restritos/files/documento/2021-04/informe_epidemiologico_02_04_2021_geral.csv",
                     encoding = 'UTF-8')
...
#DADOS DA VACINA                                             #
##############################################################
dados_vacina <- fread("https://s3-sa-east-1.amazonaws.com/ckan.saude.gov.br/PNI/vacina/uf/2021-04-02/uf%3DPR/part-00000-9aee14e7-7cd1-4ae6-9d53-53f9a25ed173.c000.csv",
                      encoding = 'UTF-8')
dados_vacina$ind <- 1
...
dados_vacina_agrup2 <- rbind(subset(dados_vacina_agrup2, vacina_descricao_dose == prim_dose),
                             subset(dados_vacina_agrup2, vacina_descricao_dose == seg_dose)[4:nrow(subset(dados_vacina_agrup2, vacina_descricao_dose == prim_dose)),])
...
bmp(file="grafico_80anosoumais.bmp",
    width=9.5, height=5, units="in", res=500)
...
obj2 <- xyplot(perc ~ semana,
               ...
               panel = function(x, y, ...) {
                 ...
                 panel.text(x=subset(dados_vacina_agrup2, vacina_descricao_dose == prim_dose)$semana,y=subset(dados_vacina_agrup2, vacina_descricao_dose == prim_dose)$perc, labels = subset(dados_vacina_agrup2, vacina_descricao_dose == prim_dose)$perc2, 
                            col = "steel blue", cex = 0.7, pos = 3, offset = 1)
                 panel.text(x=subset(dados_vacina_agrup2, vacina_descricao_dose == seg_dose)$semana,y=subset(dados_vacina_agrup2, vacina_descricao_dose == seg_dose)$perc, labels = subset(dados_vacina_agrup2, vacina_descricao_dose == seg_dose)$perc2, 
                            col = "blue", cex = 0.7, pos = 3, offset = 1)
...
dados_vacina_agrup3 <- rbind(subset(dados_vacina_agrup3, vacina_descricao_dose == prim_dose),
                             subset(dados_vacina_agrup3, vacina_descricao_dose == seg_dose)[7:nrow(subset(dados_vacina_agrup3, vacina_descricao_dose == prim_dose)),])
...
### 70 anos ou mais
Nesta faixa de idade têm-se `r total80` pessoas. Até a semana epidemiológica `r max(dados_vacina_agrup2$semana)`
, segundo dados obtidos do [Ministério da Saúde](https://opendatasus.saude.gov.br/dataset/covid-19-vacinacao), foram vacinadas `r num_vacin_prim_dose` pessoas com a 1ª dose e `r num_vacin_seg_dose` pessoas com a 2ª dose no estado do Paraná.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
bmp(file="grafico_70anos.bmp",
    width=9.5, height=5, units="in", res=500)
obj1 <- xyplot(incidencia ~ semana,
       group = grupo,
       type = "o",
...
dados_vacina_agrup4 <- rbind(subset(dados_vacina_agrup4, vacina_descricao_dose == prim_dose),
                             subset(dados_vacina_agrup4, vacina_descricao_dose == seg_dose)[7:nrow(subset(dados_vacina_agrup4, vacina_descricao_dose == prim_dose)),])
...
### 60 a 69 anos
Nesta faixa de idade têm-se `r total60` pessoas. Até a semana epidemiológica `r max(dados_vacina_agrup4$semana)`
, segundo dados obtidos do [Ministério da Saúde](https://opendatasus.saude.gov.br/dataset/covid-19-vacinacao), foram vacinadas `r num_vacin_prim_dose` pessoas com a 1ª dose e `r num_vacin_seg_dose` pessoas com a 2ª dose no estado do Paraná.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
bmp(file="grafico_60anos.bmp",
    width=9.5, height=5, units="in", res=500)
obj1 <- xyplot(incidencia ~ semana,
       group = grupo,
       type = "o",
...
dados_vacina_agrup5 <- rbind(subset(dados_vacina_agrup5, vacina_descricao_dose == prim_dose),
                             subset(dados_vacina_agrup5, vacina_descricao_dose == seg_dose)[4:nrow(subset(dados_vacina_agrup5, vacina_descricao_dose == prim_dose)),])
...
### Abaixo de 60 anos
Nesta faixa de idade têm-se `r totalmenos60` pessoas (considerados apenas do grupo de risco). Até a semana epidemiológica `r max(dados_vacina_agrup5$semana)`
, segundo dados obtidos do [Ministério da Saúde](https://opendatasus.saude.gov.br/dataset/covid-19-vacinacao), foram vacinadas `r num_vacin_prim_dose` pessoas com a 1ª dose e `r num_vacin_seg_dose` pessoas com a 2ª dose no estado do Paraná.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
bmp(file="grafico_menores60anos.bmp",
    width=9.5, height=5, units="in", res=500)
obj1 <- xyplot(incidencia ~ semana,
       group = grupo,
       type = "o",
...
![](grafico_menores60anos.bmp)

### Tabela de variação dos óbitos da semana atual em relação à anterior

```{r, echo=FALSE}
dados_obitos_perc2 <- label_percent(accuracy = 1)((dados_covid_obitos2$incidencia[max(dados_covid_obitos2$semana)]-dados_covid_obitos2$incidencia[max(dados_covid_obitos2$semana)-1])/dados_covid_obitos2$incidencia[max(dados_covid_obitos2$semana)-1])
dados_obitos_perc3 <- label_percent(accuracy = 1)((dados_covid_obitos3$incidencia[max(dados_covid_obitos3$semana)]-dados_covid_obitos3$incidencia[max(dados_covid_obitos3$semana)-1])/dados_covid_obitos3$incidencia[max(dados_covid_obitos3$semana)-1])
dados_obitos_perc4 <- label_percent(accuracy = 1)((dados_covid_obitos4$incidencia[max(dados_covid_obitos4$semana)]-dados_covid_obitos4$incidencia[max(dados_covid_obitos4$semana)-1])/dados_covid_obitos4$incidencia[max(dados_covid_obitos4$semana)-1])
dados_obitos_perc5 <- label_percent(accuracy = 1)((dados_covid_obitos5$incidencia[max(dados_covid_obitos5$semana)]-dados_covid_obitos5$incidencia[max(dados_covid_obitos5$semana)-1])/dados_covid_obitos5$incidencia[max(dados_covid_obitos5$semana)-1])
...
knitr::kable(
  ...
  align = "c",
  caption = "Variação em relação à semana anterior"
)
```
